    ...
    public partial class ShipLoad : Form
    {
        ...
        private void Ship_DataGridViewData_Route_DG_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                while (gdt.Read())
                {
                    if (Ship_DataGridViewData_Fee_DG.Rows[index].Cells[1].Value.ToString() == gdt[3].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[2].Value.ToString() == gdt[5].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[3].Value.ToString() == gdt[7].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[4].Value.ToString() == gdt[9].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[5].Value.ToString() == gdt[10].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[6].Value.ToString() == gdt[11].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[7].Value.ToString() == gdt[12].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[8].Value.ToString() == gdt[13].ToString() &&
                        Ship_DataGridViewData_Fee_DG.Rows[index].Cells[9].Value.ToString() == gdt[14].ToString()
                        )
                    {
        ...
        //TBShipLaneInfo 호출
        public void selectTBShipLaneInfo()
        {
            Connect();
            ...
            while (gdt.Read())
            {
                Ship_DataGridViewData_Fee_DG.Rows.Add(dt[8], dt[9], dt[6], dt[7], dt[13], dt[14]);
            }
            ...
            //데이터를 불러오는 공간에 해당 코드를 넣으면 정렬화를 비활성화 가능(행헤더를 클릭시 오류나는거 방지)
            foreach (DataGridViewColumn column in Ship_DataGridViewData_Fee_DG.Columns)
            {
                column.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
            Ship_DataGridViewData_Fee_DG.CurrentCell = null;
        }
        ...
        private void Ship_DataGridViewData_Route_DG_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
