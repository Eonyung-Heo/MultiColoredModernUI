...
namespace MultiColoredModernUI.Forms.Ship
{
    public partial class ShipLoad : Form
    {
        ...
        private void Ship_Fee_TB_TextChanged()
        {
            ...
            /// where 이후 조건수정하기
            string strSql = "UPDATE NEW_SHIP.dbo.TBHarbor SET StartTime = @TimeAdd, Rank = @Rank2Add, TimeRequired = @TimerequiredAdd, AdultFee = @AdultFeeAdd, YouthFee = @YouthFeeAdd, SeniorFee = @SeniorFeeAdd, ChildFee = @ChildFeeAdd where ODSayLaneID = @ODSayLaneID and ODSayShipCompanyID = @ODSayShipCompanyID";
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@ODSayLaneID", Ship_ChildFee_TB.Text);
                cmd.Parameters.AddWithValue("@ODSayShipCompanyID", Ship_ChildFee_TB.Text);
                cmd.ExecuteNonQuery();
