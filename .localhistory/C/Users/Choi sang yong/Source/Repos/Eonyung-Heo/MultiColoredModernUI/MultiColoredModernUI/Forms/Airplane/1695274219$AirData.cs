        ...
        public void Airplane_DataCrawling()
        {
            try
            {
                ...
                for (var ListCountElementsCount = 1; ListCountElementsCount <= ListCountElements.Count; ListCountElementsCount++)
                {
                    ...
                    {
                        // 이거 뭔차이지..?
                        var Data1 = _driver.FindElement(By.XPath($"//*[@id='outTbody']/tr[{RouteListCount}]/td[1]/p/span[1]/a")).Text.Replace("(", "/").Replace(")", "").Split('/');
                        var Data2 = _driver.FindElement(By.XPath($"//*[@id='outTbody']/tr[{RouteListCount}]/td[1]/p/span[2]")).Text.Replace("에서", "!").Split('!');
                        var Data3 = _driver.FindElement(By.XPath($"//*[@id='outTbody']/tr[{RouteListCount}]/td[2]")).Text;
                        var Data4 = _driver.FindElement(By.XPath($"//*[@id='outTbody']/tr[{RouteListCount}]/td[3]")).Text;
                        var Data5 = "";
                        for (var daycount = 4; daycount <= 10; daycount++)
                        {
                            string day = "";
                            IWebElement operation = _driver.FindElement(By.XPath($"//*[@id='outTbody']/tr[{RouteListCount}]/td[{daycount}]"));
                            if (operation.Text == "운항요일입니다.")
                            {
                                if (daycount == 4)
                                {
                                    day = day + "월";
                        ...
                        if (Air_DataGridViewData.InvokeRequired)
                        {
                            Air_DataGridViewData.BeginInvoke(new MethodInvoker(delegate ()
                            {
                                ...
                                Air_DataGridViewData.Rows.Add(DataList[kk][2], DataList[kk][3], DataList[kk][4], DataList[kk][5], DataList[kk][6], DataList[kk][7], DataList[kk][8], DataList[kk][9], DataList[kk][10], DataList[kk][11], DataList[kk][12], DataList[kk][13], DataList[kk][0], DataList[kk][1]);
                                kk++;
                            }));
