                    ...
                    for (int sectionnum1 = 1; sectionnum1 <= tr.Count; sectionnum1++)
                    {
                        ...
                        for (int sectionnum2 = 1; sectionnum2 < 9; sectionnum2++)
                        {
                            ...
                            else if (EndList_x != "선택하신 도착지의 항로가 없습니다.")
                            {
                                ...
                                C_Code.Add(_driver.FindElement(By.XPath($"//*[@id='pnl_Default']/table[2]/tbody/tr[1]/td[{Num}]/a")).Text); //a코드 텍스트
                                D_Code.Add(_driver.FindElement(By.XPath($"//*[@id='pnl_Text']/div[2]/table/tbody/tr[{sectionnum3}]/td[{sectionnum4}]")).Text);//b코드 텍스트
                                test_test_test.Text = _driver.FindElement(By.XPath($"//*[@id='pnl_Text']/div[2]/table/tbody/tr[{sectionnum3}]/td[{sectionnum4}]")).Text; // 자료 확인용
                                continue;
                            }
                        }
                    }
