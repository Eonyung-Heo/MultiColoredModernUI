...
namespace MultiColoredModernUI.Forms.Ship
{
    public partial class ShipAdd : Form
    {
        ...
        public void Ship_HarborSE_CB()
        {
            ...
            while (sdt.Read())
            {
                Ship_RouteAdd_DG.Rows.Add(sdt[0], sdt[1], sdt[3], sdt[4], sdt[5]);
            }
        ...
        {
            // 버튼 클릭 시 메시지 박스 표시
            DialogResult result = MessageBox.Show("선택을 계속하시겠습니까?", "확인", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

            // 사용자가 "예"를 선택했을 때의 처리
            if (result == DialogResult.Yes)
            {
                // 여기에 선택 "예"일 때의 동작을 추가
                MessageBox.Show("선택을 계속합니다!");
            }
            else
            {
                // 여기에 선택 "아니오"일 때의 동작을 추가
                MessageBox.Show("선택을 취소했습니다.");
            }
            /*
